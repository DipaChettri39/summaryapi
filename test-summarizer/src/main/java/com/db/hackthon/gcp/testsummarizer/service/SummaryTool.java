...
class SummaryTool {
	...
	void extractSentenceFromContext() {
		...
		try {
			while ((nextChar = in.read()) != -1) {
				...
				sentences.add(new Sentence(noOfSentences, (new String(temp)).trim(), (new String(temp)).trim().length(),
						noOfParagraphs));
				noOfSentences++;
				prevChar = nextChar;
			}
	...
	public String printSummary() {
		...
		for (Sentence sentence : contentSummary) {
			summary.append(sentence.value).append('.');
	...
	/*
	 * void printStats(){ System.out.println("number of words in Context : " +
	 * getWordCount(sentences)); System.out.println("number of words in Summary : "
	 * + getWordCount(contentSummary)); System.out.println("Commpression : "
	 * +getWordCount(contentSummary)/ getWordCount(sentences) ); }
	 */
